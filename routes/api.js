...
router.get('/zh-tw/index',(req,res)=>{
    ...
    let limiter = ` LIMIT ${offset}, ${pageLimit}`;
    let filter  = '';
    let getColumn = 'SELECT article.id, news.news_zhtw AS news, main_img, unixtime, title_zhtw AS title, abstract_zhtw AS abstract, viewed_count';
    let selectFromJoin = '';
    if (keyword !== undefined){
        selectFromJoin = ' FROM article INNER JOIN news ON article.news_id = news.id';
...
router.get('/zh-tw/article',(req,res)=>{
    ...
    let viewCountCache = 'view_count_'+id;
    client.incr(viewCountCache,()=>{ // view count + 1, if the article has not been viewed yet, redis generates a key and sets the value = 0
        let articleCache = 'article_'+'zh-tw_'+id;
        client.get(articleCache, function(err, reply){
            if (reply){
                // 更新 cache 中 similar article count
                let cacheReply = JSON.parse(reply);
                ...
                let getArticle = `SELECT news.news_zhtw AS news, url, title_zhtw AS title, subtitle_zhtw AS subtitle, abstract_zhtw AS abstract, author, unixtime, context_zhtw AS context, main_img, similar_article FROM article INNER JOIN news ON article.news_id = news.id WHERE article.id = ${id}`;
                mysql.conPool.getConnection((err,con)=>{
                    con.query(getArticle,(err,article)=>{
                        con.release();
                        if (err) {
                        ...
                        let similarArticle = JSON.parse(article[0].similar_article);
                        let getArticle = 'SELECT article.id, news.news_zhtw AS news, main_img, unixtime, title_zhtw AS title, abstract_zhtw AS abstract, url, viewed_count FROM article INNER JOIN news ON article.news_id = news.id ';
                        let articleID = `WHERE article.id = ${similarArticle[0]} OR article.id = ${similarArticle[1]} OR article.id = ${similarArticle[2]}`;
                        con.query(getArticle+articleID,(err,similarResult)=>{
                            // 在更新文章後還沒有算出 similar article 前會出錯 cannot read property 0 of undefined.
                            if(err){
