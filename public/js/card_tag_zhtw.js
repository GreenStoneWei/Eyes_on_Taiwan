...
/**
 * Create chinese article cards on index page
...
function createArticleCard(array, parentElement) {
	for (let i=0; i<array.length; i++) {
		...
		if (array[i].main_img === 'null' || array[i].main_img === 'undefined') {
			...
			const mainImgLink = createElement('a', ['mobile-img'], {href: `/zh-tw/view/article?id=${array[i].id}`}, cardBody);
			const mainImg = createElement('div', ['main-img'], {}, mainImgLink);
		...
		const titleLink = createElement('a', ['card-title'], {href: `/zh-tw/view/article?id=${array[i].id}`}, textBlock);
		...
		const readMore = createElement('a', ['card-link', 'read-more'], {href: `/zh-tw/view/article?id=${array[i].id}`}, readBlock);
...
/**
 * Get chinese tags by article ID
	...
	xhr.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			for (let i=0; i<tagArray.length; i++) {
				const tagNode = createElement('a', ['tag-link'], {href: `/zh-tw/?tag=${tagArray[i]}`}, tagContainer);
